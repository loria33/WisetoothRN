...
interface SutureRemovalStageInformationProps {
...
class SutureRemovalStageInformation extends React.Component<SutureRemovalStageInformationProps> {
  focusListener: any;
  scrollView: any;
  constructor(props: Readonly<SutureRemovalStageInformationProps>) {
    super(props);
  ...
  render() {
    ...
    const { sutureRemovalDate, suturesRemovedBy, atTheTimeOfSutureRemoval } = answers.sutureRemovalStageInformation;
    return (
      <Fragment>
      <SafeAreaView style={styles.saveAreaBackgrounColor} />
      <SafeAreaView style={styles.saveAreaView}>
      <View style={{ flex: 1 }}>
        <Header title={this.getHeader(visit.Installs)} navigation={navigation} showEditIcon={true} visit={{...visit, pageName: 'SutureRemovalStageInformation'}}  style={{paddingBottom:10}}/>
        <ScrollView
          ...
          <View style={styles.patientInformation}>
            ...
            <View style={[styles.column, { marginBottom: 5 }]}>
              <View style={styles.patientContent}>
                <PatientText text={suturesRemovedBy} placholder={I18n.t('suturesRemovedBy')} />
              </View>
              <View style={styles.patientContent}>
                <PatientText text={moment(sutureRemovalDate).format('DD/MM/YYYY')} placholder={I18n.t('sutureRemovalDate')} />
              </View>
              <View style={[styles.patientContent, { borderBottomWidth: 0 }]}>
                <PatientText text={atTheTimeOfSutureRemoval} placholder={I18n.t('atTheTimeOfSutureRemoval')} />
              </View>
            </View>
          </View>
        </ScrollView>
        <Footer back={this.goBack} next={this.goNext} nextPageName="SutureRemovalStage2Information"
          isFailure={visit.isFailure} pageNum={3} navigation={navigation} item={visit} />
      </View>
      </SafeAreaView>
      </Fragment>
    );
...
export default SutureRemovalStageInformation;
